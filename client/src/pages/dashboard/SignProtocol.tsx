...
export const createSchema = async (
  ...
  try {
    ...
    const schemaItems: SchemaItem[] = Object.entries(attestationJson).map(
      ([key]) => ({
        name: key,
        type: "string",
      })
...
export const createAttestation = async (
  ...
  try {
    ...
    if (signatureCount > 0) {
      for (let i = 1; i <= signatureCount; i++) {
        if (
          !signerPublicKeys[i - 1] ||
          !ethers.isAddress(signerPublicKeys[i - 1])
        ) {
    ...
    // Return the object with attestation details
    return {
      attestationId: initialAttestation.attestationId,
      txHash: initialAttestation.txHash,
      indexingValue: initialAttestation.indexingValue,
  ...
  const signAttestation = async (signerIndex: number) => {
    try {
      ...
      if (
        ...
        signerPublicKeys[signerIndex - 1].toLowerCase()
      ) {
        throw new Error(
          `Unauthorized signer. Your address (${signerAddress}) is not authorized to sign as Signer ${signerIndex}`
        );
      }
      ...
      if (attestationData[`signer${signerIndex}Signed`]) {
        throw new Error(
          `This attestation has already been signed by signer${signerIndex}`
        );
      }
      ...
      const updatedAttestation = await client.createAttestation({
        ...
        data: updatedData,
        indexingValue:
          "0xeaec938211a3ab271edb1b9fedcfa472db30c02f_0x538cfd76c4b97c5a87e1d5eb2c7d026d08d34a81",
      ...
      setStatus(
        `Attestation signed by signer${signerIndex}. New attestation ID: ${updatedAttestation.attestationId}`
      );
    } catch (error) {
  ...
  return (
    <div className="p-4">
      <h1 className="text-2xl font-bold mb-4">
        Interactive Attestation Application
      </h1>
      ...
      <Card className="mb-4">
        ...
        <CardContent>
          ...
          <TextField
            ...
            onChange={(e: React.ChangeEvent<HTMLInputElement>) =>
              setSignatureCount(Number(e.target.value))
            }
          />
          ...
          <Button className="mr-2" onClick={addSchemaField}>
            Add Field
          </Button>
          <Button onClick={createSchema}>Create Schema</Button>
        </CardContent>
        <CardContent>Schema ID: {schemaId}</CardContent>
      </Card>
      ...
      {dynamicFields.length > 0 && (
        <Card className="mb-4">
          ...
          <CardContent>
            {dynamicFields.map((field, index) => (
              <div key={index} className="mb-2">
                {!field.name.startsWith("signer") &&
                  field.name !== "status" && (
                    <TextField
                      label={field.name}
                      type={field.type === "uint256" ? "number" : "text"}
                      onChange={(e) =>
                        updateContractData(field.name, e.target.value)
                      }
                    />
                  )}
              </div>
          ...
          <CardContent>Attestation ID: {attestationId}</CardContent>
        </Card>
      )}
      ...
      {signatureCount > 0 && (
        <Card className="mb-4">
          ...
          <CardContent>
            {Array.from({ length: signatureCount }, (_, i) => i + 1).map(
              (signerIndex) => (
                <Button
                  key={signerIndex}
                  className="mr-2"
                  onClick={() => signAttestation(signerIndex)}
                >
